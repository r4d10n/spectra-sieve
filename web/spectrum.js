...
Spectrum.prototype.addWaterfallRow = function(bins) {
    ...
    if (this.wfrowcount % 100 == 0)
    {
        ...
        this.ctx_wf.font = "10px sans-serif";
...
Spectrum.prototype.doAutoScale = function(bins) {
    ...
    this.setRange(Math.ceil(minbinval * 0.050) * 10, Math.ceil(maxbinval * 0.075) * 10);  // 75% to nearest 10
...
Spectrum.prototype.incrementSpan = function() { 
    var span = { span : this.spanHz + this.tuningStep };
    this.ws.send(JSON.stringify(span));               
}

Spectrum.prototype.decrementSpan = function() { 
    var span = { span : this.spanHz - this.tuningStep };
    this.ws.send(JSON.stringify(span));               
}
...
Spectrum.prototype.onKeypress = function(e) {
    ...
    switch (e.key) {        
        ...
        case "c":
            ...
            break;
        case "ArrowUp":
            if (e.shiftKey) this.rangeUp();
            else this.incrementGain();  
            break;
        case "ArrowDown":
            if (e.shiftKey) this.rangeDown();
            else this.decrementGain();
            break;
        case "ArrowLeft":
            if (e.shiftKey) this.rangeDecrease();
            else this.decrementFrequency();
            break;
        case "ArrowRight":
            if (e.shiftKey) this.rangeIncrease();
